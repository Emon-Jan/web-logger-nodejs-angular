...
export class WebService {
   ...
   * 
   * api: http://localhost/api/summary
   * 
   ...
   */
  getWebLogSummary() {
    return this._http.get("http://" + this.host + "/api/summary");
  }
   ...
   * 
   * api: http://localhost/api/mac?from=sdate&to=edate
   * 
  ...
  getMacAddressesWithTime(sDate: Date, eDate: Date) {
    if (sDate == undefined && eDate == undefined) {
      return this._http.get("http://" + this.host + "/api/mac?from='1538360857'&to='1538922457'");
    } else {
      let sdate = parseInt((sDate.getTime() / 1000).toString());
      let edate = parseInt((eDate.getTime() / 1000).toString());
      return this._http.get("http://" + this.host + "/api/mac?from='" + sdate + "'&to='" + edate + "'");
    }
  }
   ...
   *            edate (Number: int of Unix timestamp in second)
   * 
   * api: http://localhost/api/ip?from=sdate&to=edate&mac=m
   * 
   * return Observable of Array of Json Object
   * 
   * Format: 
  ...
  getWebUsages(m: any, sDate?: Date, eDate?: Date) {
    if (sDate == undefined && eDate == undefined) {
      return this._http.get("http://" + this.host + "/api/ip?from='1538360857'&to='1538922457'&mac='" + m + "'");
    } else {
      let sdate = parseInt((sDate.getTime() / 1000).toString());
      let edate = parseInt((eDate.getTime() / 1000).toString());
      return this._http.get("http://" + this.host + "/api/ip?from='" + sdate + "'&to='" + edate + "'&mac='" + m + "'");
    }
  }
}
